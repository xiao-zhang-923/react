      ...
      <Menu.SubMenu key={key} title={<span>{icon && <Icon type={icon} />}<span>{title}</span></span>}>
        {
          subs && subs.map(item => {
            return (
              item.childrenSubs ? (
                <Menu.SubMenu key={item.key} title={<span>{icon && <Icon type={item.icon} />}<span>{item.title}</span></span>}>
                  {
                    item.childrenSubs && item.childrenSubs.map(items => {
                      return items.childrenSubs && items.childrenSubs > 0 ? this.renderSubMenu(items) : this.renderMenuItem(items)
                    })
                  }
                </Menu.SubMenu>
